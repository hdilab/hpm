...
import numpy as np
import argparse
from models.DigitalLogicHPM import DigitalLogicHPM
...
if args.file == "NA":
    ...
    L1 = DigitalLogicHPM(numBits=NumBits,
             numOnBits=NumOnBits,
             lower=L1feeder,
             printInterval=args.print,
             name="L1",
             feedbackFactor=2,
             inputThreshold = 20 * args.bits,
             contextThreshold = args.threshold,
             writer=writer)
    ...
    for i in range(2, n_layers+1):
        newLayer = DigitalLogicHPM(numBits=NumBits,
                        numOnBits=NumOnBits,
                        lower=lowerLayer,
                        printInterval=args.print,
                        name="L%s" % i,
