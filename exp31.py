...
"""
Main experiment 31
- Test of AR module with 3 Layer LSTM
"""
import numpy as np
import argparse
...
h = torch.randn(n_layers, 1, NumBits)
c = torch.randn(n_layers, 1, NumBits)

startTrainingTime = time.time()
prev = torch.rand((1,1, NumBits))
for i in range(n_epochs):
    actual = trainFeeder.feed()
    actual = torch.tensor(actual.T, dtype=torch.float32)
    actual = torch.reshape(actual, (1,1,-1))
    input = torch.reshape(actual, (1,1,-1))
    optimizer.zero_grad()
    pred, (newh, newc) = AR(prev, (h, c))
    prev = actual.detach()
    ...
    if i % TestInterval == 3:
        ...
        with torch.no_grad():
            ...
            for j in range(numTest):
                ...
                listInput = inputIndices.tolist()[0][0]
                setInput = set(listInput)
                listTopIndices = topIndices.tolist()[0][0]
